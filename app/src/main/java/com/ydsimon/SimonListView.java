...
public class SimonListView extends ListView implements OnScrollListener {
    private float mLastY = -1; // save event
    ...
    // -- 底部的View
    private SimonListViewFooter mFooterView;
    ...
    public SimonListView(Context context) {
        super(context);
        initWithContext(context);
    }

    public SimonListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initWithContext(context);
    }

    public SimonListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initWithContext(Context context) {
        ...
        // 初始化头部View
        mHeaderView = new SimonListViewHeader(context);
        ...
        // 初始化底部的View
        mFooterView = new SimonListViewFooter(context);
    ...
    public void setPullLoadEnable(boolean enable) {
        ...
        if (!mEnablePullLoad) {
            ...
            mPullLoading = false;
            mFooterView.show();// 显示
            mFooterView.setState(SimonListViewFooter.STATE_NORMAL);
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
            if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
                mHeaderView.setState(SimonListViewHeader.STATE_READY);
            } else {
                mHeaderView.setState(SimonListViewHeader.STATE_NORMAL);
            }
        }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (mEnablePullLoad && !mPullLoading) {
            if (height > PULL_LOAD_MORE_DELTA) { // 高度足以调用加载更多
                mFooterView.setState(SimonListViewFooter.STATE_READY);
            } else {
                mFooterView.setState(SimonListViewFooter.STATE_NORMAL);
            }
        }
    ...
    private void startLoadMore() {
        mPullLoading = true;
        mFooterView.setState(SimonListViewFooter.STATE_LOADING);
                ...
                if (mTouchState == TOUCH_STATE_X) {
                    ...
                    if (getFirstVisiblePosition() == 0) {
                        ...
                        if (mEnablePullRefresh
                            ...
                            mHeaderView.setState(SimonListViewHeader.STATE_REFRESHING);
                            if (mListViewListener != null) {
